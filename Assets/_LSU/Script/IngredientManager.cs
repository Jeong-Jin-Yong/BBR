...
public class IngredientManager : MonoBehaviour
{
    ...
    private void Start()
    {
        string targetBread = GameState.lastTargetBread; // "moca", "white", "sausage"

        // targetBread를 인덱스로 매핑
        switch (targetBread)
        {
            case "moca":
                target = itemDatas[0];  // MochaData
                currentIngredientGroup = groups[0];
                break;
            case "sausage":
                target = itemDatas[1];  // SausageData
                currentIngredientGroup = groups[1];
                break;
            case "white":
                target = itemDatas[2];  // WhiteBreadData
                currentIngredientGroup = groups[2];
                break;
            default:
                Debug.LogError("❌ Unknown bread type from GameState: " + targetBread);
                return;
        }
