...
public class GameManager_LSU : MonoBehaviour
{
    ...
    //진행도 업데이트
    private void ProgressTimerUpdate()
    {
        progressTimer += Time.deltaTime;
        progressSlider.value = progressTimer / progressTime;

        if(progressSlider.value <= 0.3 && stageID == 1)
        {
            stageID = 1;
        }
        else if(progressSlider.value <= 0.6)
        {
            stageID = 2;
        }
        else if(progressSlider.value <= 0.99 )
        {
            stageID = 3;
        }
        else
